...
- `tools/test_*.py` — current ROM-backed regression suite (**196 tests** at this package checkpoint).
...
1. classify the remaining technically named effect-object constructors only where their update/draw behavior proves a visual/runtime identity;
2. investigate only evidence-backed indirect writers or scene-reset behavior for the latent ending-effect argument/state;
3. hardware/mGBA-validate the newly recovered Player/NPC OAM ordering/depth behavior together with fixed8 collision/corner behavior, Level9→10 scripted entrance, foreground particles, argument-0 fifth-sketch corruption screen, and recovered Title/PDA/Wardrobe/audio timing.
