...
- `data/audio_call_sites.csv` — recovered one-shot/loop/channel-control call sites, with high-confidence action labels for state-2/state-4 activation (SFX 3), secondary social-topic cursor movement (SFX 4), generic scene portals (SFX 5), state4 dialogue terminal opcodes -1/-2/-3 (SFX 7), and the final-sketch transition (SFX 13).
...
- `tools/test_*.py` — current ROM-backed regression suite (**140 tests** at this package checkpoint).
...
The canonical audio subsystem is now reconstructed as well: all **14 original PCM resources** are byte-exact checked-in assets, the runtime has the recovered **8-channel 16,384 Hz** signed-PCM mixer with FIFO A/DMA1/Timer0/Timer1 double buffering, normal Levels 0–9 route to music ID 0, and Level 10 routes to music ID 1. High-confidence gameplay SFX routing now covers state-2 dialogue and state-4 collection activation (**SFX 3**), actual secondary social-topic cursor movement (**SFX 4**), generic scene portal activation (**SFX 5**), state4 dialogue terminal opcodes `-1/-2/-3` (**SFX 7**), and the state4 final-sketch transition (**SFX 13**, exactly once). Title START now adds the separately code-proven **SFX 6** route. Other call sites—including additional uses of IDs 3/4/5/7 and IDs 8/9/10/11/12—remain evidence-exported but unwired until their action semantics are proved. Music ID 2 is preserved as canonical data but is now code-proven **unreachable/orphaned in the public demo**: the exhaustive ROM scan finds only three loop-player calls, the reachable selector values are 0/1, and no direct one-shot or loop call targets ID 2. The one-shot entry point is now proven to consume only `(sound_id, volume)`, while the looping entry point consumes `(sound_id, mode, volume)`; gameplay music uses mode 2 and volume `0x90`. The exact 75-word fade table is relocated from ROM `0x08A8E370` to runtime `0x030013C0`, including the original counter-75 one-word-overrun lookup immediately before channel replacement.

The public-demo Wardrobe selector/preview path is now executable as the hidden B+SELECT browse-only scene, and the state 1/2/4 NPC interaction rectangles, dialogue contexts, collection selector, and rusty-key gate are executable in the clean-room runtime. The CFA reconstruction uses the recovered **BG0/BG1/BG2/BG3** layout across **all 11 levels / 13 graphics variants**, loads 241 unique non-Player physical descriptors / 290 level references plus 16 story overlays, renders exact frame-1 NPC/story graphics, follows state-3 routes, and consumes typed state 1/2/4 interaction events through `GbStoryRuntime`. The runtime now executes all **7 dialogue scripts / 58 records** with context-specific opcode handling, the two canonical Stas/Julia social profiles and recovered response tables, the six-step sketch/key sequence, primary/auxiliary message selectors, persistent consumed story pickups, canonical CFA-font BG0 dialogue/social presentation, the five-sprite monster transition, and the special Level-10 post-key traversal to Y=360/410. The dangerous state4 `-5` effect stops at `final_effect_pending`; normal-context `-5` remains a bounded generic-action hook until its argument-specific semantics are proved. Story overlays are still initially level-gated only except for persistent consumption, and there is not yet a proven Messages-screen navigation flow. Grass/leaves/foreground records remain non-rendering until their exact visuals are sufficiently proved. The highest-value next targets are:

1. emulator/frame-trace `0x08004FE0` and reconstruct the final fifth-sketch visual effect safely, including what the terminal sequence does immediately after the current `final_effect_pending` boundary;
2. finish the one special Level-9 destination `524` and recover story-overlay activation/global gates so overlay visibility follows progression rather than level alone;
...
4. recover sufficiently proved grass/leaves/foreground rendering and then recover the canonical Messages-screen controls/presentation if evidence supports them;
5. continue resolving the remaining medium-confidence SFX 8/9/10/11/12 branches; music sample 2 is closed as code-proven unreachable/orphaned in the public demo. Then hardware/mGBA-validate Title/audio/Wardrobe timing and presentation before adding content beyond the public demo.
